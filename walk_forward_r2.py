#!/usr/bin/env python
"""R2 residualized-target three-arm screen (research_r2_residual_target_prespec.md).

Minimal modification of walk_forward_v2317.py: identical folds, purge,
config, seeds, val partition and metrics. Adds --arm:
  base    raw Y_ret, 97 features            (v2.3.17 arm A; equivalence check)
  resid   per-date-demeaned Y_ret, 97 in    (arm B; macro expected to drop at selection)
  nomacro raw Y_ret, macro columns removed  (arm C)
...
RESULTS_ROOT = Path('results/research_r2')
V2317_ROOT = Path('results/walkforward_v2317')   # committed arm-A reference
...
def run_fold(data, fold_id, test_year, binding, last_fold,
    ...
    # --- R2 arm handling (everything else identical to v2.3.17) ---
    if arm == 'nomacro':
        keep_cols = ~is_macro
        ...
        print(f"    [arm nomacro] removed {n_macro_input} macro columns "
        ...
        chk = float(np.abs(pd.Series(Y_ret_tr_full).groupby(tr_dates)
                           .mean().values).max())
        print(f"    [arm resid] per-date demeaned train target: "
    ...
    fold_result = {
        ...
        'test_year': test_year,
        'binding': binding,
        'n_macro_input': n_macro_input if arm != 'nomacro' else 0,
...
def main():
    ...
    parser.add_argument('--arm', required=True,
                        choices=['base', 'resid', 'nomacro'],
                        help='R2 arm (see pre-spec)')
    ...
    if args.smoke:
        seed_tag += "_smoke"
    out_dir = RESULTS_ROOT / f"arm_{args.arm}_{seed_tag}"

    print("=" * 70)
    print(f"R2 THREE-ARM SCREEN — arm={args.arm} — {seed_tag}")
    print("=" * 70)
    ...
    # Equivalence check vs committed v2.3.17 (arm base only, not smoke)
    equivalence = None
    if args.arm == 'base' and not args.smoke and args.seed is not None:
        equivalence = {}
        for fr in fold_results:
            ref = V2317_ROOT / f"seed{args.seed}" / f"fold{fr['fold_id']}_dates.csv"
            if not ref.exists():
                continue
    ...
    if binding_frs:
        ...
        print("(R2 screen: compare arms via paired per-fold delta-IC per "
              "the R2 pre-spec anchors; this line is per-arm only.)")

    out_dir.mkdir(parents=True, exist_ok=True)
    summary = {
        'arm': args.arm,
        'prespec': 'research_r2_residual_target_prespec.md',
        'equivalence_vs_v2317': equivalence,
        'seed': args.seed,
