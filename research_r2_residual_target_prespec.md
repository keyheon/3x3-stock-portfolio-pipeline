...
## Amendment 1 Result Record (3 seeds, 2026-09-10; verdict_3seed.md via r2full_verdict.py)

**C−A on 3-seed means: mean ΔIC +0.0422, positive folds 3/5 → MIXED** (anchor requires 4/5). The effect is **seed-robust** (per-fold agreement across seeds ≤ 0.004; per-seed binding means 0.0518/0.0509/0.0512) but **year-inconsistent**: macro removal repairs the years where the base failed (2021 +0.027, 2023 +0.195, 2024 +0.010) and costs ≈0.01 in the years where the base succeeded (2022 −0.011, 2025 −0.011). The decision-map wording "not seed-robust" does not describe this outcome; the binding action is unchanged: MIXED → recorded; R3 proceeds on the raw base per its sequencing note.

**Absolute observation (hypothesis-generating; not a claim)**: the no-macro configuration's 3-seed aggregate — mean IC +0.0513, 5/5 positive years, alpha +13.18%p (not beta-adjusted), seed std 0.0004 — sits above every v2.3.17 threshold. Those thresholds were registered for a different configuration, and this one was selected on the same folds (selection-under-test), so the observation has no inferential standing on these folds. It is exactly the case for the remedy named in Amendment 1: a **new pre-registered forward hold-out gate** (train through the last selection-clean date; score 2026+ snapshots as their 63-day outcomes accrue; dependence-aware statistics; beta-adjusted alpha primary). Designing that gate is the next research item after R3 launches.
