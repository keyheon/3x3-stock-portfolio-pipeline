#!/usr/bin/env python
"""R3-full fundamentals arm (research_r3_full_prespec.md).

Built on walk_forward_r2.py (itself a minimal edit of walk_forward_v2317.py):
identical folds, purge, config, seeds, val partition and metrics.
Composable flags replace R2's --arm so the base can be whatever R2 selects:
  --target raw|resid   training target (resid = per-date demeaned, as R2 arm B)
  --drop-macro         remove macro columns before selection (R2 arm C)
  --fund               append the 9 point-in-time fundamental features from
                       results/research_r3/fund_features.npz; they bypass the
                       corr filter (var filter still applies) per the pre-spec
Test-side metrics are always computed on raw realized returns.
"""
...
RESULTS_ROOT = Path('results/research_r3/wf')
V2317_ROOT = Path('results/walkforward_v2317')   # committed v2.3.17 reference
R2_ROOT = Path('results/research_r2')            # committed R2 arm references
FUND_FILE = Path('results/research_r3/fund_features.npz')
...
MACRO_TOKENS = ('macro_', 'ff_', 'xasset_')   # same rule as the v2.3.3 cache check


def macro_flags(feat_names):
    return np.array([any(tok in f for tok in MACRO_TOKENS) for f in feat_names])


def load_data(cache_path=CACHE, fund_path=None):
    ...
    if fund_path is not None:
        fd = np.load(fund_path, allow_pickle=True)
        F = fd['F'].astype(np.float32)
        fund_names = [str(f) for f in fd['feat_names']]
        assert F.shape[0] == X.shape[0], \
            f"fund table rows {F.shape[0]} != cache rows {X.shape[0]} (built on a different cache?)"
        X = np.hstack([X, F])
        feat_names = feat_names + fund_names
        print(f"[Data] Fundamentals appended: {len(fund_names)} features from {fund_path} "
              f"(availability {F[:, -1].mean()*100:.1f}%)")
...
def run_fold(data, fold_id, test_year, binding, last_fold,
             ...
             target='raw', drop_macro=False, fund=False):
    t0 = time.time()
    ...
    is_macro = macro_flags(names)
    n_macro_input = int(is_macro.sum())
    target_std_raw = float(Y_ret_tr_full.std())

    is_fund = np.array([n.startswith('f_') for n in names])

    # --- R2/R3 arm handling (everything else identical to v2.3.17) ---
    if drop_macro:
        keep_cols = ~is_macro
        X_tr_full = X_tr_full[:, keep_cols]
        X_te = X_te[:, keep_cols]
        names = [n for n, k in zip(names, keep_cols) if k]
        is_macro = np.zeros(len(names), dtype=bool)
        is_fund = np.array([n.startswith('f_') for n in names])
        print(f"    [drop-macro] removed {n_macro_input} macro columns "
              f"(expected 43); {len(names)} features remain")
    if target == 'resid':
        tr_dates = data['sample_dates'][train_mask]
        date_mean = (pd.Series(Y_ret_tr_full).groupby(tr_dates)
                     .transform('mean').values.astype(np.float32))
        Y_ret_tr_full = Y_ret_tr_full - date_mean
        chk = float(np.abs(pd.Series(Y_ret_tr_full).groupby(tr_dates)
                           .mean().values).max())
        print(f"    [target resid] per-date demeaned train target: "
              f"std {target_std_raw:.4f} -> {Y_ret_tr_full.std():.4f}; "
              f"max |per-date mean| after = {chk:.2e}")
    target_std_train = float(Y_ret_tr_full.std())
    ...
    n_fund_would_pass = int((is_fund & keep).sum())
    if fund:
        keep = keep | (is_fund & keep_var)      # pre-spec: fundamentals bypass corr filter
    if keep.sum() < 10:
        keep = keep_var
    if keep.sum() < 10:
        keep = np.ones(X_tr_full.shape[1], dtype=bool)

    mom_idx, mom_name = pick_momentum_feature(names)
    mom_signal = X_te[:, mom_idx].copy() if mom_idx is not None else None

    X_tr_sel = X_tr_full[:, keep]
    X_te_sel = X_te[:, keep]
    n_features = int(keep.sum())
    n_macro_selected = int((is_macro & keep).sum())
    n_fund_selected = int((is_fund & keep).sum())
    print(f"    Features: {n_features} selected "
          f"(var_thr={var_thr:.4f}, corr_thr={corr_thr:.4f}); "
          f"macro selected: {n_macro_selected}/{int(is_macro.sum())}; "
          f"fund selected: {n_fund_selected}/{int(is_fund.sum())} "
          f"(would pass corr filter: {n_fund_would_pass})"
    ...
    fold_result = {
        'target': target, 'drop_macro': bool(drop_macro), 'fund': bool(fund),
        'n_fund_selected': n_fund_selected,
        'n_fund_would_pass_corr': n_fund_would_pass,
        'fold_id': fold_id,
        'test_year': test_year,
        'binding': binding,
        'n_macro_input': 0 if drop_macro else n_macro_input,
        'n_macro_selected': n_macro_selected,
        'target_std_train_raw': target_std_raw,
        'target_std_train_used': target_std_train,
        'mae_note': 'MAE not comparable across arms (target scale differs)',
...
def main():
    ...
    parser.add_argument('--target', default='raw', choices=['raw', 'resid'])
    parser.add_argument('--drop-macro', action='store_true')
    parser.add_argument('--fund', action='store_true',
                        help='append point-in-time fundamentals (R3 arm)')
    parser.add_argument('--folds', default='1,2,3,4,5',
                        help='Comma-separated fold ids to run (binding by default)')
    ...
    if args.smoke:
        seed_tag += "_smoke"
    arm_tag = f"{args.target}{'_nomacro' if args.drop_macro else ''}{'_fund' if args.fund else ''}"
    out_dir = RESULTS_ROOT / f"{arm_tag}_{seed_tag}"

    print("=" * 70)
    print(f"R3-FULL — arm={arm_tag} — {seed_tag}")
    print("=" * 70)
    ...
    data = load_data(args.cache, fund_path=FUND_FILE if args.fund else None)
    ...
    for fold_id, test_year, binding in FOLD_SPEC:
        ...
        fr, rows = run_fold(data, fold_id, test_year, binding, last_fold,
                            ...
                            target=args.target, drop_macro=args.drop_macro,
                            fund=args.fund)
    ...
    # Equivalence check vs the committed reference for this base configuration
    ref_dir = None
    if not args.fund and args.seed is not None:
        if args.target == 'raw' and not args.drop_macro:
            ref_dir = V2317_ROOT / f"seed{args.seed}"
        elif args.target == 'resid' and not args.drop_macro:
            ref_dir = R2_ROOT / f"arm_resid_seed{args.seed}"
        elif args.target == 'raw' and args.drop_macro:
            ref_dir = R2_ROOT / f"arm_nomacro_seed{args.seed}"
    equivalence = None
    if ref_dir is not None and not args.smoke:
        equivalence = {}
        for fr in fold_results:
            ref = ref_dir / f"fold{fr['fold_id']}_dates.csv"
            if not ref.exists():
                continue
            ref_df = pd.read_csv(ref).set_index('date')
            new_df = pd.DataFrame(all_rows[fr['fold_id']]).set_index('date')
            j = ref_df[['ic', 'alpha']].join(new_df[['ic', 'alpha']],
                                             lsuffix='_ref', rsuffix='_new',
                                             how='inner')
            d_ic = (j['ic_new'] - j['ic_ref']).abs()
            fold_delta = float(j['ic_new'].mean() - j['ic_ref'].mean())
            equivalence[fr['fold_id']] = {
                'n_dates_matched': int(len(j)),
                'fold_ic_ref': float(j['ic_ref'].mean()),
                'fold_ic_new': float(j['ic_new'].mean()),
                'fold_delta_ic': fold_delta,
                'mean_abs_date_delta_ic': float(d_ic.mean()),
                'max_abs_date_delta_ic': float(d_ic.max()),
                'reuse_ok': bool(abs(fold_delta) <= 0.005),
            }
            print(f"[Equivalence] fold {fr['fold_id']}: ref IC "
                  f"{j['ic_ref'].mean():+.4f} vs new {j['ic_new'].mean():+.4f} "
                  f"(fold delta {fold_delta:+.4f}; per-date mean|d| "
                  f"{d_ic.mean():.4f}, max {d_ic.max():.4f}) -> "
                  f"{'REUSE OK' if abs(fold_delta) <= 0.005 else 'RERUN ARM A'}")
    ...
    if binding_frs:
        ...
        print("(R3 screen: compare Fund - Base via paired per-fold delta-IC per "
              "the R3-full pre-spec anchors; this line is per-arm only.)")

    out_dir.mkdir(parents=True, exist_ok=True)
    summary = {
        'arm': arm_tag,
        'prespec': 'research_r3_full_prespec.md',
        'equivalence_reference': str(ref_dir) if ref_dir else None,
